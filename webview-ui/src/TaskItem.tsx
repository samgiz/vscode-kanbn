...
const TaskItem = ({
  ...
  const createdDate =
    "created" in task.metadata ? formatDate(task.metadata.created, dateFormat) : null
  const updatedDate =
    "updated" in task.metadata ? formatDate(task.metadata.updated, dateFormat) : null
  const startedDate =
    "started" in task.metadata ? formatDate(task.metadata.started, dateFormat) : null
  const dueDate = "due" in task.metadata ? formatDate(task.metadata.due, dateFormat) : null
  const completedDate =
    "completed" in task.metadata ? formatDate(task.metadata.completed, dateFormat) : null
  ...
  return (
    <Draggable key={task.id} draggableId={task.id} index={position}>
      {(provided, snapshot) => {
        ...
        return (
          ...
          >
            ...
            {task.metadata.tags !== undefined && task.metadata.tags.length > 0 && (
            ...
            {"assigned" in task.metadata && task.metadata.assigned != null && (
            ...
            {createdDate != null && (
              <div
                className="kanbn-task-data kanbn-task-data-created"
            ...
            {updatedDate != null && (
              <div
                className="kanbn-task-data kanbn-task-data-updated"
            ...
            {startedDate != null && (
              <div
                className="kanbn-task-data kanbn-task-data-started"
                title={`Started ${startedDate}`}
              >
            ...
            {dueDate != null && (
              <div className="kanbn-task-data kanbn-task-data-due" title={`Due ${dueDate}`}>
                <i className="codicon codicon-watch"></i>
                {dueDate}
              </div>
            )}
            {completedDate != null && (
              <div
                className="kanbn-task-data kanbn-task-data-completed"
                title={`Completed ${completedDate}`}
              >
            ...
            {task.relations.length > 0 &&
              task.relations.map((relation) => (
                ...
                >
                  ...
                  <span className="kanbn-task-data-label">{relation.type}</span> {relation.task}
                </div>
              ))}
