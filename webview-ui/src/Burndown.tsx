...
const Burndown = (): JSX.Element => {
  ...
  const processMessage = useCallback((event) => {
    ...
    if (
      newState.sprintMode === false &&
      newState.burndownData.series.length > 0 &&
      newState.startDate === ""
    ) {
  ...
  const refreshBurndownData = useRef(
    debounce(500, (settings) => {
      vscode.postMessage({
  ...
  const chartData =
    state.burndownData.series.length > 0
        ...
        <div className="kanbn-burndown-tooltip">
          ...
          {data.tasks.map((task) => (
            <p className="kanbn-burndown-tooltip-task" key={task.id}>
              {{ created: "Created", started: "Started", completed: "Completed" }[task.eventType]}{" "}
              {task.task.name}
            </p>
          ))}
        </div>
                ...
                >
                  {state.sprints.length > 0 ? (
                    state.sprints.map((sprint) => {
                      return (
                        <option key={sprint.start} value={sprint.name}>
                          {sprint.name}
                        </option>
                      )
                    })
