...
export default class KanbnStatusBarItem {
  ...
  constructor(context: vscode.ExtensionContext, kanbn: Kanbn | null) {
  ...
  async update(kanbn: Kanbn): Promise<void> {
    ...
    if (await this._kanbn.initialised()) {
      ...
      const text = [`$(project) ${status.tasks}`]
      let tooltip: string[] = []
      if (status.tasks > 0) {
        tooltip = [`${status.tasks} task${status.tasks === 1 ? "" : "s"}`]
        if (
          "startedTasks" in status &&
          status.startedTasks !== undefined &&
          status.startedTasks > 0
        ) {
          text.push(`$(play) ${status.startedTasks}`)
          tooltip.push(`${status.startedTasks} started task${status.startedTasks === 1 ? "" : "s"}`)
        }
        if (
          "completedTasks" in status &&
          status.completedTasks !== undefined &&
          status.completedTasks > 0
        ) {
          text.push(`$(check) ${status.completedTasks}`)
          tooltip.push(
            `${status.completedTasks} completed task${status.completedTasks === 1 ? "" : "s"}`
          )
        }
      } else {
      ...
      if (
        vscode.workspace.getConfiguration("kanbn").get("showUninitialisedStatusBarItem") === true
      ) {
        this._statusBarItem.show()
      } else {
